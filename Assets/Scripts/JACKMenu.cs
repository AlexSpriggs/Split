...
public class JACKMenu: MonoBehaviour 
{
	...
	private string clicked = "", MessageDisplayOnAbout = "Start new game \n ";
	private Rect WindowRect = new Rect((Screen.width / 2) - 100, Screen.height / 2, 200, 200);
	private float volume = 1.0f;
	private float brightness = 1.0f;
	private bool particleBool = true;
	...
	private void optionsFunc(int id)
	{
		...
		GUILayout.Box("Brightness");
		brightness = GUILayout.HorizontalSlider(brightness ,0.0f,1.0f);
		RenderSettings.ambientLight = Color.Lerp (ambientDarkest,ambientLightest,brightness);
		particleBool = GUILayout.Toggle(particleBool, "Particles");
	...
	private void Update()
	{
		if (clicked == "continue" || clicked == "credits" && Input.GetKey (KeyCode.Escape))
			clicked = "";
